...
export const AnalyticsDashboard: React.FC = () => {
  ...
  const fetchHeatmapClicks = async (page: string) => {
    setLoadingHeatmap(true);
    const apiUrl = window.location.hostname === 'localhost' ? 'http://localhost:3000' : 'https://bot.sheerit.com.co';
    try {
      const res = await fetch(`${apiUrl}/api/admin/click-heatmap?page=${encodeURIComponent(page)}`);
      const data = await res.json();
  ...
  useEffect(() => {
    if (subTab === 'traffic') {
      fetchTrafficStats();
      fetchHeatmapClicks(selectedHeatmapPage);
    }
  }, [subTab, selectedHeatmapPage]);
                    ...
                    <div className="relative">
                      ...
                      {heatmapClicks.length === 0 && (
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-slate-800/90 text-white border border-gray-700 px-4 py-2.5 rounded-xl text-center z-30 shadow-2xl">
                          ...
                          <p className="text-[9px] text-gray-400 mt-0.5">Aún no hay clics registrados en esta página.</p>
                        </div>
                      )}
                      {heatmapClicks.length > 0 && (
                        <div className="absolute bottom-4 left-4 bg-slate-900/90 text-white border border-gray-750 px-3 py-1.5 rounded-xl z-30 flex items-center gap-2 text-[10px] shadow-lg">
                          <div className="flex gap-1 items-center">
                            ...
                            <span>{heatmapClicks.length} clics graficados</span>
                          </div>
                        </div>
                      )}
                    </div>
